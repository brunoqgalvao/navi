...
Identifiers: `integration-mcp`, `integrationMcp`, `integration-status`, `integrationStatus`, `mcp-oauth-provider`, `mcpOAuthProvider`, `handleIntegrationsRoutes`. **Files:** Delete `server/routes/integrations.ts`, `server/services/integration-mcp.ts`, `server/services/integration-status.ts`, `server/services/mcp-oauth-provider.ts`, `server/integrations/` (incl. `cli.ts`; remove the `navi-integrations` bin entry from `package.json`). Modify `server/index.ts`, Settings UI (Integrations tab), and any extension panel per inventory.
**Do NOT touch:** `server/routes/mcp.ts`, `services/mcp-presets.ts`, `services/mcp-settings.ts` (plain MCP config is a keep) — **read their imports first**: if `mcp-presets`/`mcp-settings` import from the deleted services, strip those references (presets that required Navi-managed OAuth get removed from the preset list; user-configured MCP servers are untouched). `.claude/skills/integrations` (external skill) stays.
- [ ] Steps 1–6
