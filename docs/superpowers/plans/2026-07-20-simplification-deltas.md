...
```
- [ ] **Step 2: Baseline verify** — `bun run --cwd packages/navi-app check` (0 errors), `bun test server/ shared/` from `packages/navi-app`, `bash packages/navi-app/scripts/smoke.sh`. All green before any deletion.
...
- [ ] **Step 1: Inventory.** `grep -rn "native-preview\|container-preview\|port-manager-preview\|port-fixer\|preview-proxy\|worktree-preview\|services/preview" server/ shared/ --include="*.ts"` — every hit must be in a deleted file or a modify-listed file. Any surprise hit: stop, add it to this task, then proceed.
- [ ] **Step 2: Unhook `server/index.ts`** — remove the six route imports (`handleWorktreePreviewRoutes`, `handleContainerPreviewRoutes`, `handleNativePreviewRoutes`, `handlePreviewProxyRoutes`, `handlePortManagerPreviewRoutes`, `handlePortFixerRoutes`) and their dispatch blocks, and the `nativePreviewService` health-status block (`activePreviews`).
- [ ] **Step 3: Unhook `server/routes/sessions.ts`** — remove the `nativePreviewService.stopForSession(id)` call and its import.
- [ ] **Step 4: Unhook `server/routes/worktrees.ts`** — remove `cleanupWorktreePreview` import and call sites (worktree cleanup proceeds without preview teardown; there is no preview to tear down).
- [ ] **Step 5: Unhook `server/routes/background-processes.ts`** — remove the lazy `import("./services/preview" …)` call sites and any code paths that only served preview process management.
- [ ] **Step 6: Delete the ten files/dir.** `git rm -r` each.
- [ ] **Step 7: Rerun the Step 1 grep** — zero non-test hits (docs/plan hits fine).
- [ ] **Step 8: Verify + commit** — `bun run check`, `bun test server/ shared/`, `scripts/smoke.sh`; `git commit -m "deltas: remove server-side preview/dev-server stack"`.
...
- [ ] **Step 1: Inventory.** `grep -rn "marketplace\|Marketplace" server/ shared/ --include="*.ts"` and note handlers in `routes/skills.ts` for: `/import`, `/import-url`, `/generate`, `/examples`, `/export`, `/categories`.
- [ ] **Step 2: Delete `routes/marketplace.ts`**; remove its import + dispatch in `server/index.ts`.
- [ ] **Step 3: Trim `routes/skills.ts`** — delete the `/api/skills/import`, `/import-url`, `/generate`, `/examples`, `/:id/export`, `/categories` handlers and their now-unused helpers/imports. **Keep:** list, `/:id` GET/DELETE, `/document`, `/files`, `/open`, `/:id/sync`, `/sync-global`, `/scan`, `/enabled`, `/global`, `/default-enabled`, `/:id/enable`, `/projects/:id/skills(/…/enable)` — the toggle path (`enabledSkillsDb`) stays byte-identical.
- [ ] **Step 4: Trim `server/skills.ts`** of helpers only the deleted handlers used (grep each exported symbol before cutting; keep discovery/hash/sync used by agent-SDK wiring).
- [ ] **Step 5: Verify + commit** — gates; `git commit -m "deltas: remove skills marketplace/import/generate server surface"`.
...
- [ ] **Step 5: Delete the eight components.** Grep gate: `grep -rn "SkillLibrary\|SkillMarketplace\|SkillsAndAgentsLibrary\|SkillImport\|SkillEditor\|SkillCommandInstall\|ProjectSkillSelector\|SkillCard\|marketplaceApi" src/` → zero hits.
- [ ] **Step 6: Verify + commit** — gates; `git commit -m "deltas: flat SkillsPanel replaces marketplace/library/editor UI"`.
...
- [ ] Merge `simplification-deltas` into `main` (from the main checkout), rerun `bun run check` on main, notify Bruno to restart the backend, manual smoke in the live app: open browser panel with a URL, preview a markdown file from the file tree, toggle a skill per-project, open context panel.
