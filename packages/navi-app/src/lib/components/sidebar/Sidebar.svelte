<script lang="ts">
  ...
  import { api, type Project, type Session, type WorkspaceFolder, type SessionFolder, type SearchResult, type Workflow, type WorkflowGate, type WorkflowSchedule } from "../../api";
...
<aside style={sidebarCollapsed ? 'width: 56px' : `width: ${sidebarWidth}px`} class="bg-gray-50/50 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 flex flex-col hidden md:flex shrink-0 relative">
  {#if !sidebarCollapsed}
    <div 
  ...
  <div class="h-14 px-2 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
    ...
    {:else}
      ...
      <div class="flex items-center gap-0.5">
        {#if currentProject}
          <button onclick={onCreateNewChat} class="p-1.5 text-gray-400 dark:text-gray-500 hover:text-gray-900 dark:hover:text-gray-100 hover:bg-gray-200/50 dark:hover:bg-gray-700 rounded transition-all duration-200" title="New Chat">
        ...
        {/if}
        <button onclick={onCollapseToggle} class="p-1.5 text-gray-400 dark:text-gray-500 hover:text-gray-900 dark:hover:text-gray-100 hover:bg-gray-200/50 dark:hover:bg-gray-700 rounded transition-all duration-200" title="Collapse sidebar">
    ...
    {:else if !currentProject}
      ...
      <div class="px-3" data-tour="workspaces">
        <div class="flex items-center justify-between mb-2 mt-2 px-2">
          ...
          <div class="flex items-center gap-1">
            <button
              ...
              class="p-1 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-400 rounded transition-colors {$showArchivedWorkspaces ? 'bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-400' : ''}"
        ...
        {#if runningChats.length > 0}
          <div class="mt-4">
            <h3 class="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1 px-2 flex items-center gap-1.5">
            ...
            <div class="space-y-0">
              {#each runningChats as chat}
                <button onclick={() => onGoToChat(chat)} class="w-full text-left px-2 py-1.5 rounded text-gray-600 dark:text-gray-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 hover:text-gray-800 dark:hover:text-gray-200 transition-colors flex items-center gap-1.5 border-l-2 border-indigo-400 dark:border-indigo-500 ml-1">
...
<SessionStatusBadge status={$sessionStatus.get(chat.id)?.status} />
          ...
          <div class="mt-4">
            ...
            <div class="space-y-0">
              {#each needsInputChats as chat}
                <button onclick={() => onGoToChat(chat)} class="w-full text-left px-2 py-1.5 rounded text-gray-600 dark:text-gray-400 hover:bg-amber-50 dark:hover:bg-amber-900/30 hover:text-gray-800 dark:hover:text-gray-200 transition-colors flex items-center gap-1.5 border-l-2 border-amber-400 dark:border-amber-500 ml-1">
        ...
        {#if recentChats.length > 0}
          <div class="mt-4">
            <h3 class="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1 px-2">Recent Chats</h3>
            <div class="space-y-0">
              {#each recentChats.slice(0, 5) as chat}
                <button onclick={() => onGoToChat(chat)} class="w-full text-left px-2 py-1 rounded text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-700 dark:hover:text-gray-300 transition-colors flex items-center gap-1.5">
                  <svg class="w-2.5 h-2.5 text-gray-300 dark:text-gray-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path></svg>
      ...
      <div class="px-3 flex-1 flex flex-col min-h-0 overflow-hidden">
        ...
        <div class="mb-6 flex items-start gap-2 min-w-0">
          <button onclick={() => onGoToProjectDashboard?.()} class="flex-1 min-w-0 px-1 text-left hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg py-2 -my-2 transition-colors group">
            ...
            <p class="text-[11px] text-gray-400 dark:text-gray-500 truncate mt-0.5 pl-6 max-w-full" title={currentProject.path}>{currentProject.path}</p>
          </button>
          <div class="relative">
            <button
              onclick={(e) => { e.stopPropagation(); showOpenInMenu = !showOpenInMenu; }}
              class="p-1.5 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors mt-0.5"
              title="Open in..."
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>
            </button>
            {#if showOpenInMenu}
              <div class="absolute right-0 top-full mt-1 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-1 min-w-[160px] z-50">
                <button onclick={openInFinder} class="w-full px-3 py-1.5 text-left text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"></path></svg>
                  Finder
                </button>
                <button onclick={openInTerminal} class="w-full px-3 py-1.5 text-left text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2">
                ...
                <button onclick={openInVSCode} class="w-full px-3 py-1.5 text-left text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2">
        ...
        <div class="flex items-center justify-between mb-2 px-1">
          ...
          <div class="flex items-center gap-1">
            ...
            {#if $blockedCount > 0}
              <span class="flex items-center gap-1 text-[10px] text-orange-500 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/30 px-1.5 py-0.5 rounded" title="{$blockedCount} session(s) need input">
        ...
        <div class="mb-2 px-1">
          <div class="relative">
            ...
            {:else}
              <svg class="absolute left-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <input
              ...
              class="w-full text-xs pl-7 pr-2 py-1.5 bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md focus:outline-none focus:ring-1 focus:ring-gray-400 dark:focus:ring-gray-500 placeholder-gray-400 dark:placeholder-gray-500 text-gray-900 dark:text-gray-100"
            />
            {#if sidebarSearchQuery}
              <button onclick={() => sidebarSearchQuery = ""} class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-400">
          ...
          {:else}
            {#if workflowRootSessions.length > 0 || onWorkflowCreate}
              <div class="mb-3">
                ...
                {#if workflowRootSessions.length === 0}
                  <div class="px-2 py-2 text-xs text-gray-400 italic">No workflows yet</div>
                {:else}
                  <div class="space-y-1">
                    {#each workflowRootSessions as workflowSession (workflowSession.id)}
                      {@const workflow = workflowRootsBySessionId.get(workflowSession.id)!}
                      {@const workflowChildren = childrenByParent.get(workflowSession.id) || []}
                      <div class="group">
                        <div class={`relative rounded-lg border ${$session.sessionId === workflow.rootSessionId ? 'bg-white dark:bg-gray-700 border-gray-200 dark:border-gray-600 shadow-sm' : 'bg-gray-50/70 dark:bg-gray-800/60 border-gray-100 dark:border-gray-800 hover:border-gray-200 dark:hover:border-gray-700'}`}>
                  ...
                  <div class="relative">
                    ...
                    {#if sessionMenuId === sess.id}
                      <div class="absolute right-0 top-full mt-1 bg-white rounded-lg shadow-lg border border-gray-200 py-1 min-w-[140px] z-[60]">
                        <button onclick={(e) => { onToggleSessionArchive(sess, e); sessionMenuId = null; }} class="w-full px-3 py-1.5 text-left text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2">
                          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4"></path></svg>
                          {sess.archived ? 'Unarchive' : 'Archive'}
                        </button>
                        <button onclick={(e) => { onEditSession(sess, e); sessionMenuId = null; }} class="w-full px-3 py-1.5 text-left text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2">
                          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                          Rename
                        </button>
                        <button onclick={(e) => { onDuplicateSession(sess, e); sessionMenuId = null; }} class="w-full px-3 py-1.5 text-left text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2">
                        ...
                        {#if $session.sessionId === sess.id && titleSuggestionRef}
                          <button onclick={(e) => { e.stopPropagation(); sessionMenuId = null; titleSuggestionRef?.triggerSuggestion(); }} class="w-full px-3 py-1.5 text-left text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"></path></svg>
                            Suggest title
                          </button>
                        {/if}
                        <button onclick={(e) => { onToggleSessionMarkedForReview(sess, e); sessionMenuId = null; }} class="w-full px-3 py-1.5 text-left text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2">
      ...
      <div class="flex items-center justify-between text-[11px]">
        ...
        <div class="flex items-center gap-2">
          {#if $session.sessionId && $session.costUsd > 0}
            <span class="text-gray-600 dark:text-gray-400 font-mono bg-gray-200/50 dark:bg-gray-700/50 px-1.5 py-0.5 rounded border border-gray-200 dark:border-gray-600" title="Session cost">${$session.costUsd.toFixed(4)}</span>
          {:else if $session.projectId}
            ...
            {#if projectCost}
              <button 
                ...
                class="text-gray-600 dark:text-gray-400 font-mono bg-gray-200/50 dark:bg-gray-700/50 px-1.5 py-0.5 rounded border border-gray-200 dark:border-gray-600 hover:bg-gray-300/50 dark:hover:bg-gray-600/50 transition-colors"
          ...
          {/if}
          <button onclick={onSearchModal} class="p-1 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-400 transition-colors" title="Search (Cmd+K)">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
          </button>
          <button onclick={onHotkeysHelp} class="p-1 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-400 transition-colors" title="Keyboard shortcuts (?)">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
          </button>
          <button onclick={onFeedback} class="p-1 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-400 transition-colors" title="Send Feedback">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"></path></svg>
          </button>
