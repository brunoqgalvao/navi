<script lang="ts">
  ...
  $effect(() => {
    if (isOpen) {
      // Use a small timeout to ensure the dropdown opens first
...
<div class="relative" bind:this={containerRef}>
  ...
  {#if isOpen}
    <div class="absolute bottom-full left-0 right-0 mb-1 bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden z-50 max-h-64 overflow-y-auto">
      {#each models as model}
        <button
          onclick={(e) => handleSelect(model.value, e)}
          class={`w-full px-3 py-2 text-left hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-0 ${selectedModel === model.value ? 'bg-gray-50' : ''}`}
        >
