<script lang="ts">
  ...
  // Get assistant messages for display (similar to ChatView)
    ...
    <div class="flex items-center gap-3 flex-1">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2">
          <h2 class="text-base font-semibold text-gray-900">Subagent</h2>
          <span class="text-xs text-indigo-500 font-mono bg-indigo-50 px-2 py-0.5 rounded">{subagentType}</span>
          {#if isActive}
          ...
          {:else}
            <div class="flex items-center gap-1.5 px-2 py-0.5 bg-teal-100 rounded-full">
              ...
              <span class="text-xs font-medium text-teal-700">done</span>
            </div>
          {/if}
        </div>
  ...
  <div class="space-y-4 max-w-3xl mx-auto">
    ...
    {:else}
      {#each assistantMessages as msg (msg.id)}
        ...
        <!-- Message block - styled like AssistantMessage -->
        <div class="space-y-2">
          <!-- Thinking blocks -->
          ...
          {#if textContent}
            <div class="text-sm leading-relaxed text-gray-800 markdown-body relative group">
              <div class="absolute -top-2 right-0 opacity-0 group-hover:opacity-100 transition-opacity z-10">
