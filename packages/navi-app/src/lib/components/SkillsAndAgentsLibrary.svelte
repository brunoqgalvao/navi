<script lang="ts">
  ...
  import { skillsApi, agentsApi, type Skill, type Agent, type SkillCategory } from "../api";
  ...
  async function loadCategories() {
    try {
      categories = await skillsApi.listCategories();
    } catch {
  ...
  onMount(async () => {
    await Promise.all([loadSkills(), loadAgents(), loadCategories()]);
  });
...
<div class="space-y-4">
  ...
  <div class="flex items-center justify-between gap-4">
    <div class="flex items-center gap-4">
      ...
      <!-- Category Filter -->
      {#if activeTab !== "agents" && categories.length > 0}
        <select
          bind:value={filterCategory}
          class="bg-white border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-gray-900 focus:outline-none transition-colors"
        >
