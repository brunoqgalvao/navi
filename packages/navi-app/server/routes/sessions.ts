...
export async function handleSessionRoutes(
  ...
  if (sessionArchiveMatch && method === "POST") {
    ...
    if (body.archived) {
      void pruneArchivedSessionArtifacts([id]);
    }
    return json(sessions.get(id));
  }
  ...
  if (archiveAllNonStarredMatch && method === "POST") {
    ...
    const sessionsToPrune = sessions
      .listByProject(projectId, true)
      .filter((session) => session.favorite === 0 && session.archived === 0)
      .map((session) => session.id);
    sessions.archiveAllNonStarred(projectId);
    if (sessionsToPrune.length > 0) {
      void pruneArchivedSessionArtifacts(sessionsToPrune);
    }
    return json({ success: true });
  }
