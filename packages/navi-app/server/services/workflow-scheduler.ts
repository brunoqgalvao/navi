...
async function executeWorkflow(workflow: Workflow, triggerSource: "manual" | "scheduled") {
  ...
  if (!gateResult.proceed) {
    workflowRuns.create({
      ...
      started_at: now,
      completed_at: now,
      skipped_reason: gateResult.reason || "Gate returned false",
      error: null,
    });
    workflows.update(workflow.id, {
      next_run_at: calculateNextWorkflowRun(schedule) ?? null,
      last_skip_reason: gateResult.reason || "Gate returned false",
      last_error: null,
    });
    broadcastFn?.({
      ...
      reason: gateResult.reason || "Gate returned false",
    });
    return;
  }
  ...
  if (!ok) {
    workflowRuns.update(runId, {
      ...
      error: "No active WebSocket connection to trigger workflow",
    });
    workflows.update(workflow.id, {
      last_error: "No active WebSocket connection to trigger workflow",
    });
  }
}
