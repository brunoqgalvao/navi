...
async function runQuery(input: WorkerInput) {
  ...
  const canUseTool = async (
    ...
    console.error(`[Worker] canUseTool called: ${toolName}`, {
    ...
    if (permissionSettings?.autoAcceptAll) {
      console.error(`[Worker] Auto-accepting tool ${toolName} (autoAcceptAll=true)`);
      return { behavior: 'allow', updatedInput: toolInput };
    }

    if (sessionApprovedAll) {
      console.error(`[Worker] Auto-accepting tool ${toolName} (sessionApprovedAll=true)`);
      return { behavior: 'allow', updatedInput: toolInput };
    }

    if (!permissionSettings?.requireConfirmation?.includes(toolName)) {
      console.error(`[Worker] Auto-accepting tool ${toolName} (not in requireConfirmation)`);
      return { behavior: 'allow', updatedInput: toolInput };
    }
  ...
  try {
    ...
    // Always include all tools in allowedTools - we'll handle permission checks via canUseTool
    // This prevents the SDK from trying to handle permissions through its own system
    const q = query({
      prompt,
      options: {
        cwd,
        ...
        model,
        tools: allTools,
        allowedTools: allTools,  // Always allow all tools at SDK level
        permissionMode: "default",
        canUseTool,  // We handle permission checks here
    ...
    const TIMEOUT_MS = 30000; // 30 second timeout for debugging

    // Set up a timeout checker
    const timeoutChecker = setInterval(() => {
      const elapsed = Date.now() - lastMessageTime;
      if (elapsed > TIMEOUT_MS) {
        console.error(`[Worker] WARNING: No message received for ${elapsed}ms - SDK may be stuck`);
      }
    }, 10000);
    ...
    try {
      ...
      console.error(`[Worker] Error during query iteration:`, loopError);
      throw loopError;
    } finally {
      clearInterval(timeoutChecker);
    }

    console.error(`[Worker] Query loop completed successfully`);
    ...
    console.error(`[Worker] Error in runQuery:`, error);
    send({
      type: "error",
      sessionId,
